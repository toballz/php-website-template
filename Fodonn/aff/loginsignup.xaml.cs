    ...
    private async void LoginSignupForgotBtn_ClickedAsync(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            else if (btnClicked.Text == "Signup")
            {
                ...
                if (uuname.Length < 3) { err = "Input username greater than 3 characters!"; }
                else
                if ((eemail.Length < 6) || (!ETop.PregEmail.Match(eemail).Success)) { err = "Input a valid email!"; }
                else
                if (ppword.Length < 5) { err = "Your password should be greater than 6 characters!"; }
                else
